...
// nextflow run labelfree.nf -ansi-log false -with-dag labelfree.html -resume

/*
 * parameters
 * Input folder containing spectral data in mzML format.
...
params.database = "$baseDir/data/UniProt_StreptococusPyogenesSF370_UP000000750_20151113-UniProt-human-Nov26-2013-reference-reviewed-canonical-without-isoforms_plusDECOYS.fasta"
params.msgfplus = "$baseDir/data/MSGFPlus.jar"

/*
 * channels for peptide identification and peptide quantification
 * The channels contain pairs of a unique ID (base name of the mzML file) and the mzML file itself.
 * Note that the unique ID is passed through (nearly) all processes of the workflow in order to match corresponding files
 * at later stages. For example, mapping peptide IDs to peptide features in process 'id_mapping' using 'uid' as key.
 */
